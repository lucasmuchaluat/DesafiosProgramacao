...
int main(void) {
  ...
  for (int t = 1; t <= NUM_TESTS; t++) {
    ...
    if (fscanf(file, "%d %d", &p.x, &p.y) != 2) {
      fprintf(stderr, "erro ao ler p do texto %s\n", path);
    }
    if (fscanf(file, "%d %d", &a.x, &a.y) != 2) {
      fprintf(stderr, "erro ao ler a do texto %s\n", path);
    }
    if (fscanf(file, "%d %d", &b.x, &b.y) != 2) {
      fprintf(stderr, "erro ao ler b do texto %s\n", path);
    }
    if (fscanf(file, "%d", &gold) != 1) {
      fprintf(stderr, "erro ao ler gabarito do texto %s\n", path);
    }
